...
# selected_model = 'lrolandroland/llama3.1-uncensored'
# selected_model = 'llama3.1-uncensored'
...
# Function to play the spoken response
...
# Function to present CLI options
...
def main():
    ...
    try:
        ...
        print(Fore.RED + f"Error al obtener la lista de modelos de Ollama: {e}")
        exit(1)

    # Prompt the user to select a model
    print(Fore.CYAN + "Modelos disponibles en Ollama:")
    for idx, model_name in enumerate(available_models):
        print(f"{idx + 1}. {model_name}")

    selected_idx = input(Fore.CYAN + "Ingresa el número del modelo que deseas utilizar: ").strip()
    try:
        selected_model = available_models[int(selected_idx) - 1]
    except (IndexError, ValueError):
        print(Fore.RED + "Selección no válida. Usando el modelo predeterminado 'llama3.1'.")
        selected_model = 'llama3.1'

    print(Fore.CYAN + f"Has seleccionado el modelo: {selected_model}")
...
- Mantienes un tono amistoso y educativo.
